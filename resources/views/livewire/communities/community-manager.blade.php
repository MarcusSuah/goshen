<div>
    ...
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage communities and their information') }}
        </flux:subheading>
    ...
    <div class="flex justify-between items-center mb-4">
        <div class="flex space-x-4">
            ...
            <flux:select wire:model.live="districtFilter" placeholder="Filter by district">
        ...
        <flux:button wire:click="create" icon="plus-circle" variant="primary" color="teal">New Community</flux:button>
    </div>
            ...
            <tbody>
                @forelse ($communities as $community)
                    <tr
                        ...
                        <td class="px-6 py-4">COMM-{{ $community->id }}</td>
                        <td class="px-6 py-4">{{ $community->code }}</td>
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $community->name }}
                        </td>
                      <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $community->district->name ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-4 py-2 whitespace-nowrap text-center space-x-2 text-sm">
                            <button wire:click="show({{ $community->id }})"
                            ...
                            <button wire:click="edit({{ $community->id }})"
                                class="text-yellow-600 hover:text-yellow-900 focus:outline-none" title="Edit Community">
                                <svg xmlns="http://www.w3.org/2000/svg" class="inline h-5 w-5" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button wire:click="confirmDelete({{ $community->id }})"
                                class="text-red-600 hover:text-red-900 focus:outline-none" title="Delete Community">
                                <svg xmlns="http://www.w3.org/2000/svg" class="inline h-5 w-5" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @empty
                    <tr class="text-center">
                        <td colspan="7" class="px-6 py-4">No communities found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto p-4">
            <div class="w-full max-w-lg bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6">
                ...
                <form wire:submit.prevent="store" class="space-y-4">
                    ...
                    <flux:select wire:model="district_id" :label="__('District')" placeholder="Select district...">
                        @foreach ($districts as $district)
                    ...
                    <flux:input icon:trailing="map-pin" placeholder="Enter community name" type="text"
                        wire:model="name" label="Community Name" />

                    <flux:input icon:trailing="hashtag" placeholder="Community code" type="text"
                        wire:model="code" label="Community Code" readonly />

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <flux:input icon:trailing="globe-alt" placeholder="e.g. 1.9756" type="text"
                                wire:model="latitude" label="Latitude" />
                            <p class="text-xs text-gray-500 mt-1">Example: 1.9756 (negative for South)</p>
                        </div>

                        <div>
                            <flux:input icon:trailing="globe-alt" placeholder="e.g. 30.0919" type="text"
                                wire:model="longitude" label="Longitude" />
                            <p class="text-xs text-gray-500 mt-1">Example: 30.0919 (negative for West)</p>
                        </div>
                    </div>
                    ...
                    <flux:fieldset>
                        ...
                        <flux:description>Set the community status.</flux:description>
    ...
    <!-- Show Modal -->
    @if ($showModal && $selectedCommunity)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg shadow-lg w-full max-w-md mx-4 p-6">
                <div class="flex justify-between items-center border-b pb-3">
                    <h3 class="text-lg font-semibold dark:text-gray-900">Community Details</h3>
                ...
                <div class="mt-4 space-y-4">
                    <div>
                        <p class="text-sm text-gray-700">Community Name</p>
                        <p class="font-medium dark:text-gray-900">{{ $selectedCommunity->name }}</p>
                    </div>

                    <div>
                        <p class="text-sm text-gray-700">Community Code</p>
                        <p class="font-medium dark:text-gray-900">{{ $selectedCommunity->code }}</p>
                    </div>

                    <div>
                        <p class="text-sm text-gray-700">District</p>
                        <p class="font-medium dark:text-gray-900">{{ $selectedCommunity->district->name ?? 'N/A' }}</p>
                    </div>

                    <div>
                        <p class="text-sm text-gray-700">Coordinates</p>
                    ...
                    <div>
                        ...
                        <p class="font-medium dark:text-gray-900">
                            @if ($selectedCommunity->is_active)
                                <flux:badge variant="solid" color="green">Active</flux:badge>
                            @else
                                <flux:badge variant="solid" color="red">Inactive</flux:badge>
                            @endif
